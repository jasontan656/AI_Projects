...
async def handle_message(message: Message, bot: Bot) -> None:
    ...
    try:
        ...
        text_out_raw = result.agent_output.get("text", "")
        if not text_out_raw:
            raise RuntimeError("empty assistant response text")
        text_out = toolcalls.call_md_escape(text_out_raw)
    ...
    status_code = int(result.agent_output.get("status_code", 200))
    telemetry.update(
        {
            "request_id": request_id,
            "latency_ms": round(latency_ms, 3),
    ...
    log.info("telegram.handler.completed %s", json.dumps(completed_payload, ensure_ascii=False))
