...
from Kobe.SharedUtility.TaskQueue.registry import send_task, is_task_name_allowed  # 通用注册/发送接口（modules）
...
def start_task(req: TaskStart) -> JSONResponse:  # 声明 POST /task/start 端点，返回 {task_id}（libraries/modules）
    ...
    """
    if req.task == "long_io":  # 比较 req.task 是否为 'long_io'；进入分支 Branch（Branch）
        async_result = tq.long_io.apply_async(
            kwargs={
    ...
    elif req.task == "sharded_job":  # 另一路由到分片任务；进入分支 Branch（Branch）
        if not req.shard_key:  # 校验必填字段（Branch）
            raise HTTPException(status_code=400, detail="shard_key is required for sharded_job")  # 抛出 HTTP 400（libraries）
        async_result = tq.sharded_job.apply_async(
            kwargs={
        ...
        # 通用分发：如果任务名在允许列表（或未配置则开放），尝试按名称发送
        if not is_task_name_allowed(req.task):
            raise HTTPException(status_code=400, detail="Task not allowed by ALLOWED_TASKS")
