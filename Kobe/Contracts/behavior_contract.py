...
from core.adapters import build_core_schema
from core.context import ContextBridge
from core.schema import SchemaValidationError
from Contracts.toolcalls import LayoutMismatch, call_compare_tree, call_scan_tree
...
def behavior_bootstrap(
    *,
    ...
    policy_file = Path(runtime_policy_path) if runtime_policy_path else root_path / "Config" / "runtime_policy.json"
    try:
    ...
    if attach_handlers:
        try:
            message_module = import_module("TelegramBot.handlers.message")
            router = getattr(message_module, "router", None)
            if router is None:
                raise AttributeError("TelegramBot.handlers.message.router 缺失")
...
def behavior_asset_guard(repo_root: Path) -> Dict[str, Any]:
    ...
    required_dirs = ["Config", "Contracts", "core", "TelegramBot", "KnowledgeBase"]
    required_files = [
        ("Config/runtime_policy.json", "runtime_policy"),
        ("Config/prompts.zh-CN.yaml", "prompts_zh"),
        ("Config/prompts.en-US.yaml", "prompts_en"),
...
def behavior_top_entry(layout: ProjectLayout, app: Optional[Any] = None) -> TopEntryManifest:
    manifest: TopEntryManifest = {"app_py": layout["app_py"], "directories": ["infra", "core", "TelegramBot"]}
