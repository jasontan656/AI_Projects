...
async def main() -> None:
    ...
    try:
        ...
        try:
            logger.error(f"[codex] server_error: ExceptionGroup with {len(eg.exceptions)} exceptions")
            for i, exc in enumerate(eg.exceptions):
                logger.error(f"[codex] exception_{i}: {type(exc).__name__}: {exc}")
                import traceback
                logger.error(f"[codex] traceback_{i}: {traceback.format_exc()}")
        except Exception:
            pass
    except Exception as e:
        try:
            logger.error(f"[codex] server_error: {type(e).__name__}: {e}")
            import traceback
            logger.error(f"[codex] traceback: {traceback.format_exc()}")
        except Exception:
            pass
        # Don't re-raise - let the server exit gracefully
